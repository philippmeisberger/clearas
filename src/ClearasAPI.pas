...
begin
  try
    ...
    begin
      ...
      if not CopyFile(PChar(FLnkFile.BackupLnk), PChar(Path), True) then
        raise EStartupException.Create('Could not create .lnk file!');
    end  //of begin
    else
      // Failed to create new .lnk file?
      if not FLnkFile.WriteLnkFile(Path, FFilePath) then
        raise EStartupException.Create('Could not create .lnk file!');
...
begin
  ...
  try
    // Failed to create new .lnk file?
    if not FLnkFile.WriteLnkFile(FKeyPath, ANewFilePath) then
      raise EStartupException.Create('Could not create .lnk file!');
