...
begin
  ...
  with Startup do
  begin
    Startup.OnChanged := OnStartupItemChanged;
  ...
  with Context do
  begin
    Context.OnChanged := OnContextItemChanged;
...
begin
  try
    ...
    begin
      ...
      if not FileExists(Startup.Selected.FilePathOnly) then
...
begin
  try
    ...
    begin
      ...
      // Warn if file does not exist
      if not FileExists(Context.Selected.FilePathOnly) then
        FLang.MessageBox([45, NEW_LINE, 46], mtWarning);
    end  //of begin
    else
      raise Exception.Create('Unknown error!');
