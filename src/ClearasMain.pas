...
begin
  lwContext.Columns[1].Caption := TClearas.GetString(87) +' ('+IntToStr(Context.ActCount)
                                   +'/'+ IntToStr(Context.Count)+')';     //Kontextmenü-Einträge zählen
end;
...
begin
  lwList.Columns[1].Caption := TClearas.GetString(2)+'('+IntToStr(Startup.ActCount)
                               +'/'+IntToStr(Startup.Count)+')';        //aktive Programme zählen
end;
...
procedure TForm1.GetContext;                             //Kontextmenüs auslesen
var
  i: integer;
...
begin
  ...
  Context.Clear;                                     //Kontextmenü Liste leeren

  try
    if cbExpert.Checked then
       begin
       Context.OnSearchBegin := OnSearchStart;       //Events verknüpfen
       Context.OnSearch := OnSearchProgress;
       Context.OnSearchEnd := OnSearchEnd;
       Application.ProcessMessages;                  //auslagern
       Context.AddEntry();                           //Einträge suchen
       end  //of begin
    else
       Context.Load;                                 //Kontextmenü-Einträge in Liste

    lwContext.Clear;                                 //Kontextmenü-ListView leeren

    for i := 0 to Context.Count -1 do                //Kontextmenüs ausgeben
...
begin
  ...
  try                                            
    if AGetNewData then                          //Daten neu auslesen?
       begin
       Startup.Clear;                            //init bzw Reset Zähler
...
begin
  ...
  with SaveDialog do
    ...
    Filter := TClearas.GetString(36);                //Filter nur für *.reg festlegen
...
begin
  ...
  with TClearas do
    ...
    bActContext.Caption := bActivate.Caption;             //Buttons Kontextmenü
    ...
    lContext.Caption := GetString(86);                    //Liste Kontextmenü
    ...
    mmFile.Caption := GetString(11);                      //"Datei"-Menü
    ...
    mmEdit.Caption := GetString(12);                      //"Bearbeiten"-Menü
    mmContext.Caption := GetString(19);
    mmDelBackup.Caption := GetString(20);

    mmView.Caption := GetString(13);                      //"Ansicht"-Menü
...
begin
  ...
  Context := TContextList.Create;               //init Kontextmenü-Objekt
...
begin
  ...
  if (TClearas.CheckWindows or (TClearas.GetWinVersion(false)[1] in ['X','2'])) then
     ...
     mmContext.Checked := TClearas.UpdateContextPath;  //Pfad in Kontextmenü aktualisieren
     GetStartups;
     end  //of begin
...
procedure TForm1.bExportContextClick(Sender: TObject);     //Kontextmenü-Eintrag
begin                                                      //exportieren
  RegExport;
end;
...
begin
  DelBackup := Startup.DeleteBackup;                     //Backup löschen speichern
  Exported := false;
  ...
  with TClearas do
    ...
    if (MessageBox(GetString(48) +'"'+ Startup.Item.Name +'"'+ GetString(49) +^J
       ...
       MB_DEFBUTTON2) = IDYES) then                      //wirklich löschen?
       begin
       ...
       if ((not Exported) and Startup.Item.StartupUser and Startup.BackupExists) then
          Startup.DeleteBackup := (MessageBox(GetString(47), 2) = IDYES);  //Backup löschen?

       if Startup.DeleteItem() then                      //Löschen + auf Fehler prüfen
          begin
          lwList.DeleteSelected;                         //Element aus ListView löschen
          CountStartups;                                 //Zähler Refresh
          end  //of begin
       else
  ...
  Startup.DeleteBackup := DelBackup;                     //Backup löschen zurücksetzen
end;


procedure TForm1.bDeleteContextClick(Sender: TObject);     //Kontextmenü-Eintrag
begin                                                      //löschen
  with TClearas do
    begin
    if (MessageBox(GetString(85) +'"'+ Context.Item.Name +'"'+ GetString(49) +^J
       ...
       if Context.DeleteItem() then                       //löschen
          begin
          lwContext.DeleteSelected;                       //Element aus ListView löschen
          CountContext;                                   //Zähler Refresh
          end  //of begin
       else
          MessageBox(TClearas.GetString(65) + TClearas.GetString(66), 3);
...
begin
  if Startup.ChangeItemStatus() then                       //aktivieren + auf Fehler prüfen
     begin
     ...
     if mmDate.Checked then                                //Deaktivierungsdatum verfügbar?
        lwList.ItemFocused.SubItems[3] := '';              //...dann Zeitpunkt aus Liste löschen
     end  //of begin
  else
...
procedure TForm1.bActContextClick(Sender: TObject);        //Kontextmenü-Eintrag
begin                                                      //aktivieren
...
procedure TForm1.bDeactContextClick(Sender: TObject);      //Kontextmenü-Eintrag
begin                                                      //deaktivieren
  if Context.ChangeItemStatus() then
     begin
     lwContext.ItemFocused.Caption := Context.Item.GetStatus;
         ...
         if (Item.SubItems[2] = '') then  //Schlüssel fehlt
            ...
            if (IsDouble(Item.SubItems[0], Item.Index) <> -1) then
               ...
               if Startup.Item.Enabled then                       //aktiviert?
                  ...
                  pmDeactivate.Caption := TClearas.GetString(7);  //Beschriftung ändern
                  end //of begin
               else
                  begin                                           //NEIN
                  bDelete.Default := false;
                  ...
                  bActivate.Default := true;
                  pmDeactivate.Enabled := true;
                  pmDeactivate.Caption := TClearas.GetString(6);  //Beschriftung ändern
                  end; //of if
...
begin
  if (Selected and (Item <> nil)) then                  //Item selektiert?
      ...
      pmExport.Enabled := true;
      pmInfos.Enabled := true;
      pmInfos.Visible := (Item.SubItems[2] = 'Shell');  //Eigenschaften für Shell anzeigen
...
procedure TForm1.pmDeleteClick(Sender: TObject);                       //löschen
begin
  if (FTab = tsStartup) then                    //Autostart
     bDelete.Click
  else
...
begin
  ...
  try
    if OpenDialog.Execute then                              //"öffnen" Klick
       ...
       pmEdit.Enabled := true;                              //"Pfad bearbeiten" in Kontextmenü
       end;  //of begin
...
procedure TForm1.mmAddClick(Sender: TObject);              //Programm hinzufügen
var
  OpenDialog: TOpenDialog;
  Name, Params: string;
...
procedure TForm1.mmContextClick(Sender: TObject);          //Kontextmenü-Eintrag
begin
...
begin
  if (FTab = tsStartup) then              //Reiter Autostart aktiv?
     ...
     GetContext;                          //Reiter Kontextmenü aktiv?
end;


procedure TForm1.mmStandardClick(Sender: TObject);       //Standard Spaltengröße
begin
...
procedure TForm1.mmOptimateClick(Sender: TObject);     //Spaltengröße optimieren
begin
  if (FTab = tsStartup) then                           //Reiter Autostart
     begin
...
begin
  ...
  if (FTab = tsContext) then                       //Reiter Kontextmenü aktiv?
     begin
     mmAdd.Visible := false;                       //Einträge unsichtbar
     ...
     mmAdd.Visible := true;                        //Einträge sichtbar
