...
procedure TMain.GetContext;                             //Kontextmenüs auslesen
var
  i: integer;
...
begin
  ...
  with SaveDialog do
    ...
    Filter := FLang.GetString(36);                  //Filter nur für *.reg festlegen
  ...
  try
    if SaveDialog.Execute then                      //"speichern" Klick
       if (FTab = tsStartup) then
          Startup.Item.ExportItem(SaveDialog.FileName)   //"Speichern"
       else
          Context.Item.ExportItem(SaveDialog.FileName);  //"Speichern"
...
begin
  ...
  begin
    ...
    mmFile.Caption := GetString(101);                      //"Datei"-Menü
    ...
    mmEdit.Caption := GetString(102);                      //"Bearbeiten"-Menü
    mmContext.Caption := GetString(104);
    mmDelBackup.Caption := GetString(105);

    mmView.Caption := GetString(20);                      //"Ansicht"-Menü
...
procedure TMain.bExportContextClick(Sender: TObject);     //Kontextmenü-Eintrag
begin                                                      //exportieren
  RegExport;
end;
...
begin
  ...
  begin
    if (MessageBox(GetString(48) +'"'+ Startup.Item.Name +'"'+ GetString(49) +^J
       + GetString(50) + GetString(57), mtConfirm) = IDYES) then  //wirklich löschen?
      begin
        //existiert Backup?
        ...
        if ((not Exported) and Startup.Item.StartupUser and Startup.BackupExists) then
           Startup.DeleteBackup := (MessageBox(GetString(47), mtQuestion) = IDYES);  //Backup löschen?

        //Löschen + auf Fehler prüfen
...
procedure TMain.bDeleteContextClick(Sender: TObject);     //Kontextmenü-Eintrag
begin                                                      //löschen
  with FLang do
  begin
    // wirklich löschen?
...
procedure TMain.bActContextClick(Sender: TObject);        //Kontextmenü-Eintrag
begin                                                      //aktivieren
...
procedure TMain.bDeactContextClick(Sender: TObject);      //Kontextmenü-Eintrag
begin                                                      //deaktivieren
  if Context.ChangeItemStatus() then
  begin
    lwContext.ItemFocused.Caption := Context.Item.GetStatus(FLang);
...
procedure TMain.pmDeleteClick(Sender: TObject);                       //löschen
begin
  if (FTab = tsStartup) then                    //Autostart
     bDelete.Click
  else
...
procedure TMain.mmContextClick(Sender: TObject);          //Kontextmenü-Eintrag
begin
...
procedure TMain.mmStandardClick(Sender: TObject);       //Standard Spaltengröße
begin
...
procedure TMain.mmOptimateClick(Sender: TObject);     //Spaltengröße optimieren
begin
  if (FTab = tsStartup) then                           //Reiter Autostart
     begin
